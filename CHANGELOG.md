...
## [0.6.0]
### Added
- `docs/02-harness-engineering.md`에 "5대 기둥의 구체 구현 매핑" 표 신설 —
  루프 쪽 5+1 부품과 균형을 맞춰, 각 기둥의 원칙을 실제 구현(명령·산출물)으로 연결.
- 기둥3 가드레일(격리)에 워크트리(worktree)를 브랜치 분기와 구분되는 구체 메커니즘으로 추가.
- 기둥4 관찰가능성에 상태 파일(`progress-template.md`)을 세션 간 관찰가능성의
  최소 구현으로 명시.
- 하네스 문서와 루프 문서가 워크트리·상태 파일을 공유 산출물로 상호 참조하도록 연결.
